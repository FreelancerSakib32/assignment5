...
function renderIssues() {
    ...
    filteredIssues.forEach(issue => {
        ...
        // Author Name Fix (Handles both object and string from API)
        ...
        if (issue.labels && issue.labels.length > 0) {
            labelsHTML = issue.labels.map(label => {
                ...
                return `<span class="flex items-center px-3 py-1 text-[11px] font-extrabold uppercase tracking-wide rounded-full border ${badgeClass}">${iconHTML}${label}</span>`;
            }).join('');
        } else {
            labelsHTML = `<span class="px-3 py-1 text-[11px] font-extrabold uppercase tracking-wide rounded-full bg-gray-100 text-gray-600 border border-gray-200">NO LABEL</span>`;
        }
        ...
        const cardHTML = `
            <div class="bg-white rounded-xl shadow-sm hover:shadow-md transition-shadow duration-200 flex flex-col h-full cursor-pointer relative overflow-hidden border border-gray-200" onclick="openIssueModal('${issueId}')">
                ...
                <div class="p-5 flex flex-col h-full mt-1">
                    ...
                    <div class="flex justify-between items-start mb-3">
                        <span class="flex items-center gap-1.5 px-2.5 py-1 text-xs font-bold rounded-full ${isClosed ? 'bg-purple-50 text-purple-700 border-purple-200' : 'bg-green-50 text-green-700 border-green-200'} border">
                            <img src="${statusIcon}" class="w-3.5 h-3.5"> <span class="capitalize">${issue.status}</span>
                        </span>
                        <span class="px-3 py-1 text-xs font-bold rounded-full bg-orange-50 text-orange-600 border border-orange-200 capitalize">
                            ${priorityText}
                        </span>
                    </div>

                    <h2 class="text-lg font-bold text-gray-900 leading-tight mb-2 line-clamp-2 hover:text-blue-600">${issue.title}</h2>
                    
                    <p class="text-sm text-gray-500 mb-4 line-clamp-3 leading-relaxed">${issue.description || 'No description provided.'}</p>

                    <div class="flex flex-wrap gap-2 mb-4 flex-grow content-start">
                        ${labelsHTML}
                    </div>

                    <div class="mt-auto pt-4 border-t border-gray-100 flex flex-col gap-1.5 text-xs">
...
async function openIssueModal(id) {
    ...
    try {
        ...
        // Author Name Fix for Modal
        const authorName = issue.author?.name || issue.author || 'Unknown';

        content.innerHTML = `
            <div class="mb-6">
                <h3 class="text-2xl font-extrabold text-gray-900 mb-3">${issue.title}</h3>
            ...
            <div class="bg-gray-50 p-6 rounded-xl border border-gray-200 mb-6">
                <h4 class="font-extrabold text-gray-800 mb-3 text-lg">Description</h4>
                <p class="text-gray-600 whitespace-pre-wrap text-[15px] leading-relaxed">${issue.description || 'No details provided.'}</p>
            </div>
            
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-y-5 gap-x-6 text-sm bg-blue-50/40 p-6 rounded-xl border border-blue-100">
                <div>
                    <span class="text-gray-500 block mb-1 text-xs uppercase tracking-wide font-bold">Author</span>
                    <strong class="text-gray-900 text-base flex items-center gap-2">
                        Post by ${authorName}
                    </strong>
                </div>
                <div>
                    <span class="text-gray-500 block mb-1 text-xs uppercase tracking-wide font-bold">Created At</span>
                    <strong class="text-gray-900 text-base">📅 ${dateStr}</strong>
                </div>
                <div>
                    <span class="text-gray-500 block mb-1 text-xs uppercase tracking-wide font-bold">Labels</span>
                    <strong class="text-gray-900 text-base text-blue-600">🏷️ ${issue.labels?.join(', ') || 'None'}</strong>
                </div>
                <div>
                    <span class="text-gray-500 block mb-1 text-xs uppercase tracking-wide font-bold">Issue ID</span>
                    <strong class="text-gray-900 text-xs font-mono bg-gray-200 px-2 py-1 rounded">#${issue._id || issue.id}</strong>
                </div>
            </div>
        ...
        content.innerHTML = `
            <div class="text-center py-10">
                ...
                <p class="text-red-600 text-lg font-bold">Failed to load issue details.</p>
                <p class="text-gray-500 text-sm mt-2">The API might be down or the ID is invalid.</p>
            </div>
